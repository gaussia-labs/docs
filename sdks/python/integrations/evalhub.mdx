...
A common integration policy is:
...
For metric-by-metric details, use the [Metrics overview](/sdks/python/metrics/overview) and the individual metric pages.

## Know which metrics need models

Not every Gaussia metric has the same runtime profile.

- `humanity` can run without an external judge model
...
For the full dependency and model setup, see:
...
from your_bridge import load_context_persistance_payload
from your_provider import ProviderConfig, run_gaussia_benchmark
    ...
    def run_benchmark_job(self, job_spec, callbacks):
        callbacks.report_status(
            JobStatusUpdate(
                ...
                progress=0.05,
                message=MessageInfo(
                    message="Loading context payload",
                    message_code="initializing",
                ),
            )
        )

        payload = load_context_persistance_payload(
            job_spec.parameters["context_persistance"]
        )
        ...
        callbacks.report_status(
            JobStatusUpdate(
                ...
                progress=0.35,
                message=MessageInfo(
                    message=f"Running {job_spec.benchmark_id}",
...
In production, adapters usually add MLflow run logging, richer evaluation metadata, and optional artifact export around this core flow.
